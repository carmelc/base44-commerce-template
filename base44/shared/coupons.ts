/**
 * Coupon validation and discount application (WooCommerce semantics).
 * Validation runs the ordered checks from the plan; application supports
 * percent / fixed_cart / fixed_product with Woo's eligibility rules.
 */
 ...
 * subtotal (Woo default). Line discounts never exceed the line's remaining value.
 */
...
async function findCoupon(sr: any, couponLine: any): Promise<any | null> {
  if (couponLine.coupon_id) {
    try {
      const c = await sr.entities.Coupon.get(couponLine.coupon_id);
      if (c) return c;
    } catch { /* fall through to code lookup */ }
  }
  const matches = await sr.entities.Coupon.filter({ code: (couponLine.code || "").toLowerCase() }, undefined, 1);
