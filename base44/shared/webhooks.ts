...
export async function dispatch(sr: any, topic: string, payload: unknown): Promise<void> {
  try {
    const hooks = (await sr.entities.Webhook.filter({ status: "active", topic })) ?? [];
...
/** Deliver to a single webhook record; used by dispatch and admin-webhooks test/redeliver. */
export async function deliverOne(sr: any, hook: any, topic: string, payload: unknown): Promise<any> {
  ...
  try {
    delivery = await sr.entities.WebhookDelivery.create({
      webhook_id: hook.id,
      topic,
  ...
  try {
    if (delivery) {
      await sr.entities.WebhookDelivery.update(delivery.id, {
    ...
    await sr.entities.Webhook.update(hook.id, patch);
  } catch (e) {
