...
/**
 * Transition `order` to `newStatus`, applying every lifecycle side effect exactly
...
export async function transitionOrder(sr: any, order: any, newStatus: string, opts: TransitionOpts = {}): Promise<any> {
  ...
  switch (newStatus) {
    ...
    case "failed":
      ...
      break;
    case "refunded":
      // stock restocking is handled per-refund by commerce/admin-refunds; coupons stay counted
  ...
  await sr.entities["commerce.Order"].update(order.id, patch);
...
async function isVirtualOnly(sr: any, order: any): Promise<boolean> {
  ...
  for (const line of lines) {
    try {
      const product = await sr.entities["commerce.Product"].get(line.product_id);
...
export async function grantDownloadPermissions(sr: any, order: any): Promise<number> {
  ...
  for (const line of order.line_items || []) {
    ...
    try { product = line.product_id ? await sr.entities["commerce.Product"].get(line.product_id) : null; } catch { /* skip */ }
    try { variation = line.variation_id ? await sr.entities["commerce.ProductVariation"].get(line.variation_id) : null; } catch { /* skip */ }
    ...
    for (const dl of src.downloads || []) {
      await sr.entities["commerce.DownloadPermission"].create({
        order_id: order.id,
...
export async function revokeDownloadPermissions(sr: any, order: any): Promise<void> {
  try {
    await sr.entities["commerce.DownloadPermission"].deleteMany({ order_id: order.id });
  } catch (e) {
...
/**
 * Strip internals before handing an order to its customer (commerce/storefront-account,
