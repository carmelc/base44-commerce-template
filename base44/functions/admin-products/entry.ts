/**
 * admin-products — product + variation mutations with side effects
...
import { deriveStockStatus } from "../../shared/stock.ts";
import { sendStockEmail } from "../../shared/emails.ts";
...
/** price/on_sale from the sale window (Woo derivation). Mutates rec. */
...
async function ensureUniqueSlug(sr: any, slug: string, selfId?: string): Promise<string> {
  let candidate = slug;
  for (let i = 2; i < 100; i++) {
    const hits = (await sr.entities.Product.filter({ slug: candidate }, undefined, 2)) ?? [];
...
async function assertUniqueSku(sr: any, sku: string, opts: { productId?: string; variationId?: string }): Promise<void> {
  if (!sku) return;
  const prods = (await sr.entities.Product.filter({ sku }, undefined, 2)) ?? [];
  ...
  const vars = (await sr.entities.ProductVariation.filter({ sku }, undefined, 2)) ?? [];
...
async function save(sr: any, payload: any): Promise<any> {
  ...
  const prev = product.id ? await sr.entities.Product.get(product.id) : null;
  ...
  let saved: any;
  if (prev) {
    await sr.entities.Product.update(prev.id, fields);
    saved = { ...prev, ...fields };
  } else {
    saved = await sr.entities.Product.create(fields);
  }

  await adjustTermCounts(sr, "ProductCategory", prev?.category_ids ?? [], saved.category_ids ?? []);
  await adjustTermCounts(sr, "ProductTag", prev?.tag_ids ?? [], saved.tag_ids ?? []);
  if ((prev?.shipping_class_id ?? "") !== (saved.shipping_class_id ?? "")) {
    if (prev?.shipping_class_id) await bumpCount(sr, "ShippingClass", prev.shipping_class_id, -1);
    if (saved.shipping_class_id) await bumpCount(sr, "ShippingClass", saved.shipping_class_id, +1);
  }
  ...
  if (Array.isArray(variations)) {
    ...
    const existing = await scanAll(sr.entities.ProductVariation, { product_id: saved.id });
...
async function diffVariations(sr: any, parent: any, incoming: any[], outThreshold: number): Promise<any[]> {
  const existing = await scanAll(sr.entities.ProductVariation, { product_id: parent.id });
  const incomingIds = new Set(incoming.filter((v) => v.id).map((v) => v.id));
  const out: any[] = [];

  for (const stale of existing.filter((v: any) => !incomingIds.has(v.id))) {
    await sr.entities.ProductVariation.delete(stale.id);
  }
  ...
  for (const v of incoming) {
    ...
    const { id: _id, created_date: _cd, updated_date: _ud, created_by: _cb, ...fields } = rec;
    if (prev) {
      await sr.entities.ProductVariation.update(prev.id, fields);
      out.push({ ...prev, ...fields });
    } else {
      out.push(await sr.entities.ProductVariation.create(fields));
    }
  }
  return out;
}
...
async function rollUpParentStock(sr: any, parent: any, variations: any[]): Promise<void> {
  ...
  if (status !== parent.stock_status) {
    await sr.entities.Product.update(parent.id, { stock_status: status });
...
async function remove(sr: any, id: string): Promise<any> {
  const product = await sr.entities.Product.get(id);
  if (!product) throw new HttpError(404, "Product not found", "not_found");

  const variations = await scanAll(sr.entities.ProductVariation, { product_id: id });
  for (const v of variations) await sr.entities.ProductVariation.delete(v.id);

  await adjustTermCounts(sr, "ProductCategory", product.category_ids ?? [], []);
  await adjustTermCounts(sr, "ProductTag", product.tag_ids ?? [], []);
  if (product.shipping_class_id) await bumpCount(sr, "ShippingClass", product.shipping_class_id, -1);

  await sr.entities.Product.delete(id);
...
async function duplicate(sr: any, id: string): Promise<any> {
  const source = await sr.entities.Product.get(id);
  ...
  const created = await sr.entities.Product.create(copy);
  ...
  for (const v of variations) {
    ...
    await sr.entities.ProductVariation.create(vCopy);
    copied++;
  }

  // duplicated product participates in taxonomy counts too
  await adjustTermCounts(sr, "ProductCategory", [], created.category_ids ?? []);
  await adjustTermCounts(sr, "ProductTag", [], created.tag_ids ?? []);
  if (created.shipping_class_id) await bumpCount(sr, "ShippingClass", created.shipping_class_id, +1);
...
async function setStock(sr: any, payload: any): Promise<any> {
  ...
  const product = await sr.entities.Product.get(id);
  if (!product) throw new HttpError(404, "Product not found", "not_found");
  const variation = variation_id ? await sr.entities.ProductVariation.get(variation_id) : null;
  ...
  const entity = variation ? "ProductVariation" : "Product";
...
async function categoryWithDescendants(sr: any, rootId: string): Promise<Set<string>> {
  const all = await scanAll(sr.entities.ProductCategory, null, "menu_order");
...
async function search(sr: any, payload: any): Promise<any> {
  ...
  let rows = await scanAll(sr.entities.Product, Object.keys(query).length ? query : null, sort);
  if (q) {
    rows = rows.filter((p) =>
