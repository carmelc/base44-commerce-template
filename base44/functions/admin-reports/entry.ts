/**
 * admin-reports — on-demand report aggregation over orders.
 * Fine to ~10k orders; larger stores should materialize stats
 * (see implementation-guidelines.md §Reports performance).
 *
...
Deno.serve(async (req) => {
  try {
    ...
    switch (action) {
      ...
      case "products-totals": return ok(await groupCount(sr.entities.Product, "type"));
      case "customers-totals": return ok(await customersTotals(sr));
      case "coupons-totals": return ok(await groupCount(sr.entities.Coupon, "discount_type"));
      case "reviews-totals": return ok(await groupCount(sr.entities.ProductReview, "status"));
      case "categories-totals": return ok(await termTotals(sr.entities.ProductCategory));
      case "tags-totals": return ok(await termTotals(sr.entities.ProductTag));
...
/** An order counts toward sales when it was paid (Woo report semantics). */
...
function finalizeNet(agg: SalesAgg): void {
  // Woo: net = gross − refunds − tax − shipping
...
async function salesAggregation(sr: any, dateMin?: string, dateMax?: string, interval?: string): Promise<{ totals: SalesAgg; series: Array<{ period: string } & SalesAgg> }> {
  const orders = (await scanAll(sr.entities.Order)).filter(
    (o) => isCounted(o) && inRange(o.created_date, dateMin, dateMax),
  );
  // refunds counted by refund date (Woo behavior)
  const refunds = (await scanAll(sr.entities.OrderRefund)).filter(
...
async function topSellers(sr: any, payload: any): Promise<any> {
  ...
  for (const row of top as any[]) {
    try {
      const p = await sr.entities.Product.get(row.product_id);
...
async function stockReport(sr: any): Promise<any> {
  ...
  const variations = await scanAll(sr.entities.ProductVariation);
...
async function ordersTotals(sr: any): Promise<Record<string, number>> {
  const orders = await scanAll(sr.entities.Order, null, "-created_date", { fields: ["id", "status"] });
...
async function customersTotals(sr: any): Promise<any> {
  const customers = await scanAll(sr.entities.Customer, null, "-created_date", { fields: ["id", "is_guest", "is_paying_customer"] });
  return {
...
async function attributesTotals(sr: any): Promise<any> {
  const attrs = await scanAll(sr.entities.ProductAttribute);
  const terms = await scanAll(sr.entities.ProductAttributeTerm, null, "-created_date", { fields: ["id", "attribute_id"] });
