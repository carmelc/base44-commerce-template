/**
 * storefront-catalog — public catalog browsing API.
 *
...
import { getSettings, getSetting, storefrontSafeSettings } from "../../shared/settings.ts";
...
import { CURRENCIES } from "../../shared/data/currencies.ts";
...
async function listProducts(sr: any, p: any): Promise<any> {
  ...
  let products = await scanAll(sr.entities.Product, { status: "publish" }, "-created_date", 5000);
...
async function categoryWithDescendants(sr: any, rootId: string): Promise<Set<string>> {
  const all = await scanAll(sr.entities.ProductCategory, {}, undefined, 2000);
...
async function getProduct(sr: any, p: any): Promise<any> {
  ...
  if (p.id) {
    try { product = await sr.entities.Product.get(String(p.id)); } catch { product = null; }
  } else if (p.slug) {
    product = (await sr.entities.Product.filter({ slug: String(p.slug) }, undefined, 1))?.[0] ?? null;
  }
  ...
  const [variations, categories, tags] = await Promise.all([
    product.type === "variable"
      ? sr.entities.ProductVariation.filter({ product_id: product.id }, "menu_order", 500)
      : Promise.resolve([]),
    resolveMany(sr.entities.ProductCategory, product.category_ids),
    resolveMany(sr.entities.ProductTag, product.tag_ids),
  ]);
  ...
  const reviews = (await sr.entities.ProductReview.filter(
...
async function productSummaries(sr: any, ids: string[] | undefined): Promise<any[]> {
  ...
  for (const id of (ids ?? []).slice(0, 20)) {
    try {
      const prod = await sr.entities.Product.get(id);
...
async function listCategories(sr: any): Promise<any> {
  const all = await scanAll(sr.entities.ProductCategory, {}, "menu_order", 2000);
...
async function listAttributes(sr: any): Promise<any> {
  const attributes = await scanAll(sr.entities.ProductAttribute, {}, undefined, 500);
  const out: any[] = [];
  for (const attr of attributes) {
    const terms = (await sr.entities.ProductAttributeTerm.filter({ attribute_id: attr.id }, "menu_order", 500)) ?? [];
    out.push({ ...attr, terms });
  }
...
async function getStoreInfo(sr: any): Promise<any> {
  ...
  const gateways = (await sr.entities.PaymentGateway.filter({ enabled: true }, "order", 50)) ?? [];
  return {
...
async function submitReview(sr: any, p: any): Promise<any> {
  ...
  let product: any = null;
  try { product = productId ? await sr.entities.Product.get(productId) : null; } catch { product = null; }
  ...
  const created = await sr.entities.ProductReview.create({
...
async function hasPurchased(sr: any, email: string, productId: string): Promise<boolean> {
  const customer = (await sr.entities.Customer.filter({ email }, undefined, 1))?.[0];
  if (!customer) return false;
  const orders = await scanAll(sr.entities.Order, { customer_id: customer.id }, "-created_date", 1000);
