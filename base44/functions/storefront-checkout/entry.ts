/**
 * storefront-checkout — public checkout API.
 *
 * Actions:
...
import { HttpError, getCallerUser } from "../../shared/auth.ts";
...
import { checkPurchasable, releaseExpiredHolds } from "../../shared/stock.ts";
import { transitionOrder, serializeOrderForCustomer } from "../../shared/orders.ts";
import { generateOrderKey, nextOrderNumber } from "../../shared/sequence.ts";
...
function fail(e: unknown): Response {
  ...
  console.error("storefront-checkout error:", e);
...
async function placeOrder(sr: any, req: Request, user: any, payload: any): Promise<any> {
  ...
  // (2b) revalidate coupons — a stale coupon fails checkout loudly (Woo behavior)
  ...
  const gateway = (await sr.entities.PaymentGateway.filter({ slug: gatewaySlug }, undefined, 1))?.[0];
  ...
  const now = Date.now();
  const order = await sr.entities.Order.create({
  ...
  // (7) cart consumed
  try { await sr.entities.Cart.update(cart.id, { status: "converted" }); } catch { /* best-effort */ }
...
async function confirmPayment(sr: any, payload: any): Promise<any> {
  ...
  if (order.customer_id) {
    try {
      const customer = await sr.entities.Customer.get(order.customer_id);
      if (customer) await bumpCustomerStats(sr, customer, order);
    } catch { /* stats can be recomputed by admin-tools */ }
  }
...
async function loadOrderByKey(sr: any, orderId: string, orderKey: string): Promise<any> {
  ...
  let order: any = null;
  try { order = await sr.entities.Order.get(orderId); } catch { order = null; }
...
async function upsertCustomer(sr: any, billing: any, shippingAddress: any, user: any): Promise<any> {
  ...
  const existing = (await sr.entities.Customer.filter({ email }, undefined, 1))?.[0];
  const base = {
  ...
  if (existing) {
    ...
    await sr.entities.Customer.update(existing.id, patch);
    return { ...existing, ...patch };
  }
  return await sr.entities.Customer.create({
    email,
    ...base,
    username: "",
    user_id: user?.id ?? "",
...
/** Denormalized customer aggregates (recount available via admin-tools). */
async function bumpCustomerStats(sr: any, customer: any, order: any): Promise<void> {
  try {
    await sr.entities.Customer.update(customer.id, {
