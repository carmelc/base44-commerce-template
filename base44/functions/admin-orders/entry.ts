/**
 * admin-orders — order creation, editing, status lifecycle, notes, emails,
...
import { ORDER_STATUSES, transitionOrder } from "../../shared/orders.ts";
import { releaseExpiredHolds } from "../../shared/stock.ts";
  ...
  try {
    ...
    switch (action) {
      ...
      case "recalculate": {
        ...
        await sr.entities.Order.update(order.id, fields);
      ...
      case "delete-note": {
        await sr.entities.OrderNote.delete(payload.note_id);
      ...
      case "revoke-download": {
        await sr.entities.DownloadPermission.delete(payload.permission_id);
...
async function getOrder(sr: any, orderId: string): Promise<any> {
  ...
  const order = await sr.entities.Order.get(orderId);
...
async function search(sr: any, payload: any): Promise<any> {
  ...
  let rows = await scanAll(sr.entities.Order, status ? { status } : null, sort);
  if (q) {
    rows = rows.filter((o) =>
...
async function addNote(sr: any, payload: any, actor: string): Promise<any> {
  const order = await getOrder(sr, payload.order_id);
  const note = await sr.entities.OrderNote.create({
    order_id: order.id,
    note: payload.note ?? "",
...
async function grantDownload(sr: any, payload: any): Promise<any> {
  const order = await getOrder(sr, payload.order_id);
  const product = await sr.entities.Product.get(payload.product_id);
  ...
  for (const dl of product.downloads) {
    created.push(await sr.entities.DownloadPermission.create({
      order_id: order.id,
...
async function remove(sr: any, orderId: string): Promise<any> {
  ...
  await sr.entities.OrderNote.deleteMany({ order_id: order.id }).catch(() => {});
  await sr.entities.OrderRefund.deleteMany({ order_id: order.id }).catch(() => {});
  await sr.entities.DownloadPermission.deleteMany({ order_id: order.id }).catch(() => {});
  await sr.entities.Order.delete(order.id);
