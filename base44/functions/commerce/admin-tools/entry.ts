...
Deno.serve(async (req) => {
  try {
    ...
    console.error("commerce/admin-tools error:", e);
...
async function recountTerms(sr: any): Promise<any> {
  ...
  for (const term of await scanAll(sr.entities["commerce.ProductAttributeTerm"])) {
    ...
    if ((term.count ?? 0) !== want) {
      await sr.entities["commerce.ProductAttributeTerm"].update(term.id, { count: want });
      terms++;
    }
  }
...
async function recountCouponUsage(sr: any): Promise<any> {
  const orders = await scanAll(sr.entities["commerce.Order"], { coupon_usages_counted: true });
  ...
  let updated = 0;
  for (const coupon of await scanAll(sr.entities["commerce.Coupon"])) {
    const uses = usage[coupon.code] ?? [];
    if ((coupon.usage_count ?? 0) !== uses.length) {
      await sr.entities["commerce.Coupon"].update(coupon.id, { usage_count: uses.length, used_by: uses });
      updated++;
    }
  }
...
async function recalcAllCustomerStats(sr: any): Promise<any> {
  const customers = await scanAll(sr.entities["commerce.Customer"]);
  const orders = await scanAll(sr.entities["commerce.Order"]);
  ...
  for (const c of customers) {
    ...
    if (patch.orders_count !== (c.orders_count ?? 0) || patch.total_spent !== (c.total_spent ?? 0)) {
      await sr.entities["commerce.Customer"].update(c.id, patch);
      updated++;
    }
  }
...
async function pruneDeliveries(sr: any, payload: any): Promise<any> {
  ...
  const deliveries = await scanAll(sr.entities["commerce.WebhookDelivery"], null, "created_date");
  let deleted = 0;
  for (const d of deliveries) {
    if (d.created_date < cutoff) {
      await sr.entities["commerce.WebhookDelivery"].delete(d.id);
      deleted++;
    }
  }
...
async function clearCarts(sr: any, payload: any): Promise<any> {
  ...
  const carts = await scanAll(sr.entities["commerce.Cart"], null, "created_date");
  let deleted = 0;
  for (const cart of carts) {
    const stale = (cart.updated_date ?? cart.created_date) < cutoff;
    if (stale || cart.status === "converted") {
      await sr.entities["commerce.Cart"].delete(cart.id);
      deleted++;
    }
  }
  return { deleted, cutoff };
...
async function regenPermissions(sr: any, payload: any): Promise<any> {
  ...
  await sr.entities["commerce.Order"].update(order.id, { download_permissions_granted: granted > 0 });
  return { granted };
}
