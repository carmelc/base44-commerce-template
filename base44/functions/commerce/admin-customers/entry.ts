/**
 * commerce/admin-customers — customer CRUD, invites and denormalized stats.
 *
  ...
  try {
    ...
    switch (action) {
      ...
      case "search": {
        ...
        let rows = await scanAll(sr.entities["commerce.Customer"], null, sort);
        if (q) {
          rows = rows.filter((c) =>
    ...
    console.error("commerce/admin-customers error:", e);
...
async function getCustomer(sr: any, id: string): Promise<any> {
  ...
  const customer = await sr.entities["commerce.Customer"].get(id);
...
async function save(sr: any, customer: any): Promise<any> {
  ...
  const dupes = (await sr.entities["commerce.Customer"].filter({ email }, undefined, 2)) ?? [];
  ...
  if (id) {
    ...
    await sr.entities["commerce.Customer"].update(id, fields);
    saved = { ...prev, ...fields };
    await dispatch(sr, "customer.updated", saved);
  } else {
    saved = await sr.entities["commerce.Customer"].create({
...
async function remove(sr: any, payload: any): Promise<any> {
  ...
  if (payload.reassign_orders_to_guest !== false) {
    const orders = await scanAll(sr.entities["commerce.Order"], { customer_id: customer.id }, "-created_date", { fields: ["id"] });
    for (const o of orders) await sr.entities["commerce.Order"].update(o.id, { customer_id: "" });
  }
  await sr.entities["commerce.Customer"].delete(customer.id);
...
async function invite(base44: any, sr: any, payload: any): Promise<any> {
  ...
  try {
    ...
    if (userId) {
      await sr.entities["commerce.Customer"].update(customer.id, { user_id: userId, is_guest: false });
 ...
 * their refunds; orders_count excludes cancelled/failed (standard commerce semantics).
 */
async function recalcStats(sr: any, customer: any): Promise<any> {
  const orders = await scanAll(sr.entities["commerce.Order"], { customer_id: customer.id });
  ...
  await sr.entities["commerce.Customer"].update(customer.id, patch);
