/**
 * commerce/admin-reviews — review moderation; keeps product rating aggregates in sync.
 *
...
Deno.serve(async (req) => {
  try {
    ...
    const getReview = async (id: string) => {
      const review = await sr.entities["commerce.ProductReview"].get(id);
    ...
    switch (action) {
      case "set-status": {
        ...
        await sr.entities["commerce.ProductReview"].update(review.id, { status: payload.status });
      ...
      case "update": {
        ...
        await sr.entities["commerce.ProductReview"].update(review.id, patch);
        const aggregates = await recalcProductRating(sr, review.product_id);
        return ok({ review: { ...review, ...patch }, aggregates });
      }
      case "delete": {
        const review = await getReview(payload.id);
        await sr.entities["commerce.ProductReview"].delete(review.id);
        const aggregates = await recalcProductRating(sr, review.product_id);
    ...
    console.error("commerce/admin-reviews error:", e);
