/**
 * admin-coupons — coupon CRUD with code normalization/uniqueness and webhooks.
 *
  ...
  try {
    ...
    switch (action) {
      ...
      case "search": {
        ...
        let rows = await scanAll(sr.entities.Coupon, null, sort);
...
async function save(sr: any, coupon: any): Promise<any> {
  ...
  const dupes = (await sr.entities.Coupon.filter({ code }, undefined, 2)) ?? [];
  ...
  let saved: any;
  if (id) {
    const prev = await sr.entities.Coupon.get(id);
    if (!prev) throw new HttpError(404, "Coupon not found", "not_found");
    await sr.entities.Coupon.update(id, fields);
    saved = { ...prev, ...fields };
    await dispatch(sr, "coupon.updated", saved);
  } else {
    saved = await sr.entities.Coupon.create({ usage_count: 0, used_by: [], ...fields });
...
async function remove(sr: any, id: string): Promise<any> {
  const coupon = await sr.entities.Coupon.get(id);
  if (!coupon) throw new HttpError(404, "Coupon not found", "not_found");
  await sr.entities.Coupon.delete(id);
