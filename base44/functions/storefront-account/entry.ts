/**
 * storefront-account — customer self-service API.
 *
 * Two access modes:
...
import { serializeOrderForCustomer } from "../../shared/orders.ts";
...
function fail(e: unknown): Response {
  ...
  console.error("storefront-account error:", e);
...
async function customerByEmail(sr: any, email: string): Promise<any | null> {
  return (await sr.entities.Customer.filter({ email: String(email).toLowerCase() }, undefined, 1))?.[0] ?? null;
}
...
Deno.serve(async (req: Request) => {
  try {
    ...
    switch (action) {
      ...
      case "order-notes": {
        const order = await orderByKey(sr, payload.order_id, payload.order_key);
        const notes = (await sr.entities.OrderNote.filter(
      ...
      case "my-downloads": {
        requireUser(user);
        const permissions = (await sr.entities.DownloadPermission.filter(
      ...
      case "get-download": {
        ...
        let perm: any = null;
        try { perm = await sr.entities.DownloadPermission.get(permissionId); } catch { perm = null; }
        ...
        await sr.entities.DownloadPermission.update(perm.id, patch);
