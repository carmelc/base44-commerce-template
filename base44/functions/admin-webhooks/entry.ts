/**
 * admin-webhooks — test pings and redelivery for webhook endpoints.
...
import { deliverOne } from "../../shared/webhooks.ts";
...
Deno.serve(async (req) => {
  try {
    ...
    switch (action) {
      case "test": {
        const hook = await sr.entities.Webhook.get(payload.webhook_id);
      ...
      case "redeliver": {
        const delivery = await sr.entities.WebhookDelivery.get(payload.delivery_id);
        if (!delivery) throw new HttpError(404, "Delivery not found", "not_found");
        const hook = await sr.entities.Webhook.get(delivery.webhook_id);
    ...
    console.error("admin-webhooks error:", e);
