/**
 * storefront-cart — public cart API (guest + member).
...
function fail(e: unknown): Response {
  ...
  console.error("storefront-cart error:", e);
...
async function touch(sr: any, cart: any, patch: Record<string, any> = {}): Promise<void> {
  ...
  await sr.entities.Cart.update(cart.id, full);
...
async function loadProductPair(sr: any, productId: string, variationId?: string): Promise<{ product: any; variation?: any }> {
  ...
  if (variationId) {
    try { variation = await sr.entities.ProductVariation.get(variationId); } catch { variation = undefined; }
...
async function mergeUserCarts(sr: any, cart: any, user: any): Promise<void> {
  ...
  const others = (await sr.entities.Cart.filter({ customer_email: user.email, status: "active" }, "-updated_date", 20)) ?? [];
  let merged = false;
  for (const other of others) {
    ...
    try { await sr.entities.Cart.update(other.id, { status: "abandoned" }); } catch { /* best-effort */ }
  }
  if (merged) {
...
Deno.serve(async (req: Request) => {
  try {
    ...
    switch (action) {
      case "create": {
        const cart = await sr.entities.Cart.create({
          cart_token: uuid(),
...
async function addItem(sr: any, cart: any, payload: any): Promise<void> {
  ...
  await sr.entities.Cart.update(cart.id, { items, expires_at });
  cart.items = items;
