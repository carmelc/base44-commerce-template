...
- `examples/commerce/implementation-guidelines.md` — Base44 Commerce template: 24 `commerce.*` entities, 14 `commerce/*` backend functions (storefront + admin APIs), the shared commerce engine under `base44/shared/commerce/`, and the store admin UI mounted at `/admin`. Read that file before working on store features.
```
