...
- **Admin UI** (`admin/`) — a React/Tailwind/shadcn admin mirroring WooCommerce's information architecture: dashboard, orders, products, coupons, customers, reports, full settings, webhooks. Admin-role gated.
...
├── admin/            React admin UI (copy into your app's src/)
├── docs/
...
2. **Push the schema and functions:**
   ...
   ```
3. **Copy the admin UI:** `admin/` → `src/admin/`.
4. **Install UI deps** (if not already present) and confirm shadcn primitives:
   ```bash
   npm i sonner recharts
   ```
   See [`admin/README.md`](./admin/README.md) for the exact shadcn component list.
...
1. Use `write_file` to copy every file under `base44/` and `admin/` into the target app (use `list_directory`/`read_file` to adapt to the app's actual layout — e.g. the `@/api/base44Client` path and your router file).
